{{-- resources/views/movements/index.blade.php (Hesap sütunu kaldırıldı, üstte hesap etiketi) --}}
@extends('layouts.app')
...
<section class="content">
  <div class="container-fluid">
    <div class="card card-outline card-primary">
      ...
      {{-- Hesap etiketi (giriş yapan kullanıcı) --}}
      <div class="px-3 py-2 bg-light border-bottom">
        <strong>Hesap:</strong>
        {{ auth()->user()->name
           ?? auth()->user()->customer_name
           ?? (auth()->user()->customer->customer_name ?? auth()->user()->email) }}
      </div>

      {{-- Filtre Formu --}}
      <form method="GET" class="p-3 border-bottom filter-form">
        <div class="row g-2">
          <div class="col-sm"><input type="date" name="from" value="{{ request('from') }}" class="form-control" placeholder="Başlangıç Tarihi"></div>
          <div class="col-sm"><input type="date" name="to"   value="{{ request('to') }}"   class="form-control" placeholder="Bitiş Tarihi"></div>
          <div class="col-sm">
          ...
          <div class="col-sm"><input type="text" name="q" value="{{ request('q') }}" class="form-control" placeholder="Açıklamada Ara"></div>
          ...
          <table class="table table-hover mb-0 align-middle">
            ...
            <tbody>
              ...
              @forelse($movements as $m)
                @php $delta=$m->movement_type==='Debit'? -$m->amount : $m->amount; $running+=$delta; @endphp
                <tr>
              ...
              @empty
                <tr><td colspan="6" class="text-center">Hiç hareket bulunamadı.</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
...
.filter-form button{margin-top:5px}
</style>
@endpush
